...
warpImage(const Image<T>& image, Image<T>& warpedImage, const Map& map)
{
	...
	.expand(inv(image.width(), image.height()))
	.expand(inv(0,		   image.height()));
  //cerr << bbox.width() << 'x' << bbox.height() << endl;
    warpedImage.resize(size_t(bbox.height()), size_t(bbox.width()));
    warpedImage = 0;
    
  // 変形後の左上隅が(0，0)になるように原点を移動する．
    Matrix<element_type>	Ht = transpose(map);
    Ht[2] += bbox.min(0) * Ht[0] + bbox.min(1) * Ht[1];

  // 変形画像の各画素値を計算する．
    const element_type	width1	= image.width()  - 1;
    const element_type	height1	= image.height() - 1;
		...
		const Map& map)
{
    ...
    {
	const auto	line0 = image0[v];
	auto		lineR = result[v + offset_v];
    ...
    for (size_t v = 0; v < warpedImage.height(); ++v)
    {
	const auto	line1 = warpedImage[v];
	auto		lineR = result[v + offset_v];
	for (size_t u = 0; u < warpedImage.width(); ++u)
      ...
      bool refine, typename MAP::element_type intensityThresh)
{
  ...
  // 点対応を示す画像を生成する．
    ...
    if (refine)
    {
	typename ICIA<MAP>::Parameters	params;
	params.intensityThresh	= intensityThresh;
	params.niter_max	= 1000;
	
	ICIA<MAP>		registration(params);
	registration.initialize(images[0]);
	element_type		err = registration(images[0], images[1], map);
...
main(int argc, char* argv[])
{
	...
	switch (mapType)
	{
	  case PROJECTIVE:
	    ...
	    break;
	  default:
	    doJob<Affinity22<element_type> >(images, surfParams, matchParams,
					     refine, intensityThresh);
	    break;
	}
    }
