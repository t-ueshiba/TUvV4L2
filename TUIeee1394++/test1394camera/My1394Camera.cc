...
My1394Camera::draw()
{
  // 必用なら YUV -> RGB の変換を行ってから画像を表示．
	...
	break;
      case MONO_8:
      case RAW_8:
	if ((bayerTileMapping() != Ieee1394Camera::YYYY) && (pixelFormat() != RAW_8))
	    gdk_draw_rgb_image(_canvas->window,
			       _canvas->style
	...
	break;
      ...
      case SIGNED_MONO_16:
      case RAW_16:
	if ((bayerTileMapping() != Ieee1394Camera::YYYY) && (pixelFormat() != RAW_16))
	    gdk_draw_rgb_image(_canvas->window,
			       _canvas->style
				      ->fg_gc[GTK_WIDGET_STATE(_canvas)],
			       0, 0, width(), height(),
...
My1394Camera::save(std::ostream& out) const
{
	...
	{
	    if ((pixelFormat() == MONO_16) || (pixelFormat() == SIGNED_MONO_16) ||
		(pixelFormat() == RAW_16))
	    {
		const u_short*	p = (u_short*)_buf;
		u_char*		q = _buf;
